...
//       .map(file => {
//         const noExtName = file.filename.replace(/\.json$/, ""); // remove .json
//         return {
//           ...file,
//           name: noExtName // ✅ add display name without extension
//         };
//       });
...
app.post("/save-json", (req, res) => {
  try {
    ...
    const id = uuidv4(); // always use UUID
    const displayName = filename || `data_${Date.now()}`;
    const fileName = `${id}.json`; // actual saved file name = id.json
    const filePath = path.join(folderPath, fileName);
    ...
    const newFileInfo = {
      id,
      filename: fileName,       // actual stored file
      name: displayName,              // ✅ user-friendly name (without .json)
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
    meta.push(newFileInfo);
...
// ✅ 3. Get data from a particular file by ID
app.get("/file/:id", (req, res) => {
  try {
    const meta = readMeta();
    const fileMeta = meta.find((m) => m.id === req.params.id);
...
app.put("/file/:id", (req, res) => {
  try {
    ...
    // ✅ Update metadata
    if (newDisplayName) {
      fileMeta.displayName = newDisplayName; // only update display name, file stays by id
    }
    fileMeta.updatedAt = new Date().toISOString();
    writeMeta(meta);
