...
app.put("/file/:id", (req, res) => {
  try {
    const host = getEffectiveHost(req);
    const { domainDir, metaFilePath } = getDomainPaths(host);
    const { data, newDisplayName, newFileName } = req.body; // Accept newFileName
    ...
    if (!fs.existsSync(oldFilePath)) return sendResponse(res, false, null, "File not found on disk", 404);

    // Rename file on disk if newFileName is provided
    if (newFileName && newFileName !== fileMeta.filename) {
      const newFilePath = path.join(domainDir, newFileName);

      // Check if new file name already exists
      if (fs.existsSync(newFilePath)) {
        return sendResponse(res, false, null, "File with new name already exists", 400);
      }

      fs.renameSync(oldFilePath, newFilePath);
      fileMeta.filename = newFileName; // Update filename in metadata
    }

    // Update file content
    const finalFilePath = path.join(domainDir, fileMeta.filename);
    ...
    // Write updated metadata back to fileIndex.json
    writeMeta(metaFilePath, meta);
